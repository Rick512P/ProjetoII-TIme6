...
int controller(int op, int *state, Memorias **memoriaInst, int tamLinhas, int **regs, Memorias **md, int *program_counter, type_instruc **instrucoesDecodificadas, RegistradoresAux *aux){
    ...
    case 1:
        ...
        {   
            ...
            (*instrucoesDecodificadas)[*program_counter] = Memoria(aux, 0); //DECODIFICA A INSTRUCAO
            ...
            if ((strcmp(instrucoesDecodificadas[*program_counter]->opcode,"0000")) == 0) // ADD/SUB/OR/AND
            {
                ...
                aux->registradorResult = ULA(instrucoesDecodificadas, program_counter, md, regs); //ULA GERARA SINAL PARA ACESSAR A MEMORIA E ENTAO PEGAR O DADO
                escritaRegistradores(regs, aux->registradorResult, instrucoesDecodificadas[*program_counter]->rt);
            }
    ...
    case 2:
        //Run by Step
            ...
            strcpy(aux->registradorInst, memoriaInst[*program_counter]->instruc);
            (*instrucoesDecodificadas)[*program_counter] = Memoria(aux, 0);
